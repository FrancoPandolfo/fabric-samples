...
func (s *SmartContract) GetRecetasPorDniYEstado(ctx contractapi.TransactionContextInterface, dni string, estado string, pageSize int32, bookmark string) ([]*Receta, string, error) {
	if dni == "" || estado == "" {
		return nil, "", fmt.Errorf("el dni y el estado son obligatorios")
	}
	if pageSize <= 0 {
		return nil, "", fmt.Errorf("pageSize debe ser mayor a 0")
	}

	// Query CouchDB para filtrar por dni y estado
	queryString := fmt.Sprintf(`{
        "selector": {
            "patientDocumentNumber": "%s",
            "status": "%s"
        }
    }`, dni, estado)
	...
	if err != nil {
		return nil, "", fmt.Errorf("error al obtener datos del ledger con paginado: %v", err)
	}
	defer resultsIterator.Close()
	...
	// Devuelve las recetas y el bookmark para la próxima página
	return recetasFiltradas, responseMetadata.Bookmark, nil
}
